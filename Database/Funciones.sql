...
BEGIN
    ...
    SELECT CONCAT(nombre_egresado,' ',apellidos_egresado)
...
BEGIN
    ...
    SELECT es.estado_laboral
    INTO estado_lab
    FROM encuesta_seguimiento es
    INNER JOIN seguimiento_egresado se
        ON es.id_encuesta = se.id_encuesta
...
SELECT fn_total_postulaciones(1);
SELECT fn_total_ofertas_empresa(25200);
SELECT fn_promedio_salario(1);
SELECT fn_nombre_completo(1);
SELECT fn_nombre_carrera(1);
SELECT fn_nombre_empresa(25200);
SELECT fn_total_egresados_carrera(1);
SELECT fn_total_encuestas();
SELECT fn_estado_laboral_actual(1);
SELECT fn_ultima_empresa(1);